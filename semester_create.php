...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Check if a record with the same semester name, but different start and end years already exists
        $sql = "SELECT * FROM semesters WHERE semester_name = '$semester_name' AND (start_year != '$start_year' OR end_year != '$end_year')";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            echo '<script type="text/javascript">';
            echo ' alert("A semester with the same name but different date range is not allowed delete the existing first");';
            echo ' window.location.href = "semester_create.php";';
            echo '</script>';
            exit;
        ...
        $sql = "INSERT INTO semesters ( semester_name, start_year, end_year) VALUES ('$semester_name', '$start_year', '$end_year')";
...
<body>
    <div class="container mt-3">
        <h1 style="  text-shadow: 3px 2px 3px rgba(0, .5, 0, .80)"
            class="fw-bolder text-center text-warning mt-3 text-outline">ADD SEMESTER</H1>
        ...
        <form method="post">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="start_year" name="start_year" min="2022" max="2099"
                    required>
            </div>
